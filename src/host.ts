...
function appendOutput(op: any, text: string): void {
  ...
  // totalLen is a monotonic cumulative append counter, so consumers can keep a
  // cursor valid across the tail-truncation above (see readOutput in
  // marketJobHooks): cursor math uses cumulative coordinates, never string
  // indexes into the truncated buffer.
  op.totalLen = (op.totalLen || 0) + String(text).length
}
...
interface Op {
  ...
  status: string
  output: string
  /** Cumulative bytes ever appended; unlike `output` it never truncates. */
  totalLen: number
 ...
 * Single-flight chokepoint: every entry point (web install/update routes, the
 * market_install/market_update tools, and startMarketJob's begin()) funnels
 * through here, so the busy check below closes the race window once for all of
 * them — callers may still pre-check for a fast refusal, but mutual exclusion
 * is guaranteed here, not at the call sites. */
function startOp(kind: string, profile: string, target: string, label: string, explicitBin: string, initialOutput?: string): { ok: boolean; busy?: boolean; opId?: string; error?: string } {
  if (activeOp && activeOp.status === 'running') return { ok: false, busy: true, error: '已有任务进行中：' + activeOp.label }
  ...
  const op: Op = {
    ...
    totalLen: String(initialOutput || '').length,
    exitCode: null,
    bin,
    hot: false,
...
function mapGitHubItem(item: any): any {
...
async function searchGitHub(
  ...
  if (hit !== undefined && Date.now() - hit.at < SEARCH_TTL_MS) return hit.data
 ...
 * An explicit `loader` argument overrides the hotCtx lookup (test injection).
 */
function liveLoaderStates(loader?: any): Map<string, { state: number | undefined; disabled: boolean }> | null {
  if (loader === undefined) {
    try {
...
/**
 * Build the JobHooks that wrap one op for ctx.jobs (pure apart from the op
 * itself — extracted so the hooks semantics are testable without a jobs
 * service). cancel() only kills when this op is STILL the live activeOp: an
 * old job's late cancel must never terminate a newer op that has since taken
 * the single-flight slot. done resolves through op.settled into a JobOutcome.
 * readOutput() is a consuming cursor in CUMULATIVE coordinates (op.totalLen):
 * appendOutput tail-truncates op.output at 200 KB, so a cursor held in
 * buffer-index coordinates would go stale (and could even re-emit or skip
 * text) after the truncation; converting through totalLen keeps the delta
 * exact for the whole op lifetime.
 */
function marketJobHooks(op: Op): { cancel(): void; done: Promise<{ status: 'completed' | 'killed' | 'failed'; detail?: string; output?: string }>; readOutput(): string } {
  // 0 = "never read": the first readOutput() emits from the very beginning
  // (including the header line startOp seeds), matching a fresh consumer.
  let cursorTotal = 0
  return {
    cancel: () => {
      // Scope the kill to THIS op: killOp() acts on whatever activeOp points
      // at, and a stale job cancel must not murder a successor op.
      if (activeOp === op && op.status === 'running') killOp()
    },
    done: op.settled.then((v: { status: string; exitCode: number | null }) => {
    ...
    readOutput: (): string => {
      // Convert the cumulative cursor into the truncated buffer's coordinates:
      // the oldest retained cumulative position is totalLen - output.length.
      const base = op.totalLen - String(op.output || '').length
      const from = Math.max(cursorTotal, base)
      const text = String(op.output || '').slice(from - base)
      cursorTotal = op.totalLen
      return text
    },
  }
}
 ...
 * single-flight pipeline: activeOp mutual exclusion applies to both (startOp
 * itself is the chokepoint), and the UI progress pill shows the tool job too.
 *
...
function startMarketJob(jobs: any, exec: any, kind: 'market-install' | 'market-update', label: string, begin: () => string): { jobId: string } | { error: string } {
  try {
    const jobId = jobs.start({
      ...
      run: () => {
        const opId = begin() // throws on refusal → nothing registered
        const op = activeOp
        // The op begin() just started must be the one we wrap; anything else
        // would mean the single-flight invariant is broken.
        if (!op || op.id !== opId) throw new Error('任务启动异常：op 不匹配（' + opId + '）')
        return marketJobHooks(op)
      },
    })
    ...
    async execute(args: any, exec: any) {
      ...
      if (jobs !== undefined) {
        ...
        const started = startMarketJob(jobs, exec, 'market-install', spec, () => {
          ...
          // startOp is the single-flight chokepoint: a busy refusal (another op
          // slipped in between the pre-checks above and here) must surface as a
          // model-readable throw, not a silently dead job.
          if (r.busy) throw new Error((r.error || '已有任务进行中') + '（等待完成后再试）')
    ...
    async execute(args: any, exec: any) {
      ...
      if (jobs !== undefined) {
        ...
        const started = startMarketJob(jobs, exec, 'market-update', name, () => {
          ...
          // startOp is the single-flight chokepoint (same busy→throw contract
          // as market_install above).
          if (r.busy) throw new Error((r.error || '已有任务进行中') + '（等待完成后再试）')
    ...
    handler: async (req: any, res: any) => {
      try {
        ...
        if (method === 'restart') {
          ...
          if (!sameOrigin(req)) return sendJson(res, 403, { ok: false, error: 'untrusted origin' })
          if (!isLoopback(req)) return sendJson(res, 403, { ok: false, error: 'restart requires a direct loopback client' })
