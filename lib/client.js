...
`;
		...
		*/
		function apiOp(method, params, timeoutMs) {
			const opts = {
				method: "POST",
				headers: { "Content-Type": "application/json" },
				body: JSON.stringify(Object.assign({ method }, params || {}))
			};
			if (typeof AbortSignal !== "undefined" && AbortSignal.timeout) try {
				opts.signal = AbortSignal.timeout(timeoutMs || 3e4);
			} catch {}
			return fetch("/api/dsh-market", opts).then(async (r) => {
				try {
			...
			const step = () => {
				...
				apiOp("op", { opId }).then((r) => {
					...
					if (r && r.ok === false && r.error) {
						if (pollFailCount < 5) {
							pollFailCount += 1;
							setTimeout(step, Math.min(3e3 * Math.pow(2, pollFailCount), 15e3));
							return;
						}
						setOpState((prev) => prev && prev.opId === opId ? {
							...
							status: "failed",
							ok: false,
							output: "与服务失去连接（" + String(r.error) + "），任务可能在后台仍在进行，刷新页面可查看状态"
						} : prev);
						return;
					}
					pollFailCount = 0;
		...
		function executeOpState(binPath) {
			...
			apiOp(cur.kind === "uninstall" ? "uninstall" : cur.kind === "update" ? "update" : "install", params, 36e4).then((r) => {
				if (gen !== opGen) {
					if (r && r.ok && r.opId) try {
						apiOp("kill", { opId: r.opId });
					} catch {}
					return;
				}
				if (!r || !r.ok) {
		...
		function killCurrentOp() {
			opGen++;
			apiOp("kill", op && op.opId ? { opId: op.opId } : {}).then((r) => {
				if (r && r.ok) {
					setOpState({
						phase: "done",
						status: "killed",
		...
		const STR = {
			...
			en: {
				...
				restartTimeout: "Restart timed out — refresh the page manually",
		...
		function pollReload(onGiveUp) {
			...
			fetch(window.location.origin + "/", { cache: "no-store" }).then((r) => {
				...
				else setTimeout(() => pollReload(onGiveUp), 2e3);
			}).catch(() => {
				setTimeout(() => pollReload(onGiveUp), 2e3);
			});
		}
		...
		function useModalInert(active) {
			(0, react.useEffect)(() => {
				if (active) return modalInert();
			}, [active]);
		}
		...
		*  SSR guard mirrors modalInert's (the client only runs in a browser). */
		function portalModal(tree) {
			return typeof document !== "undefined" ? (0, react_dom.createPortal)(tree, document.body) : tree;
		}
		function GlobalProgress() {
			const [, force] = (0, react.useState)(0);
			const [restarting, setRestarting] = (0, react.useState)(false);
			const [restartError, setRestartError] = (0, react.useState)(null);
			...
			const requestRestart = () => {
				...
				apiOp("restart").then((r) => {
					if (r && r.ok) {
						pollReload(() => {
							setRestarting(false);
							setRestartError(t("restartTimeout"));
						});
						return;
					}
					setRestarting(false);
			...
			return (0, react.createElement)("div", { className: "mkts mkts-prog" }, needRestart ? (0, react.createElement)("div", { className: "mkts-restart" }, (0, react.createElement)("span", { style: { flex: 1 } }, restartError ? (0, react.createElement)("span", { className: "mkts-live-bad" }, restartError) : t("restartBanner")), (0, react.createElement)("button", {
		...
		function MarketPanel(props) {
			...
			const loadInstalledRef = (0, react.useRef)(loadInstalled);
			...
			const [togglingSet, setTogglingSet] = (0, react.useState)({});
			...
			(0, react.useEffect)(() => {
				if (modalOpen) return modalInert();
			}, [modalOpen]);
			(0, react.useEffect)(() => {
				if (!modalOpen) return;
			...
			const modal = op && !op.minimized ? portalModal((0, react.createElement)("div", {
			...
			(0, react.useEffect)(() => {
				if (!toast) return;
				const iv = setTimeout(() => setData((d) => d.toast === toast ? {
					...d,
					toast: null
				} : d), 6e3);
				return () => {
			...
			return (0, react.createElement)("div", {
				...
				onClick: () => goPage(shownPage - 1)
			}, t("prev")), (0, react.createElement)("span", { className: "mkts-pager-info" }, shownPage + " / " + totalPages), (0, react.createElement)("button", {
				className: "mkts-cmdbtn",
		...
		function MarketOnboarding(props) {
			...
			const [done, setDone] = (0, react.useState)(readDone);
			(0, react.useEffect)(() => {
			...
			if (done) return null;
			return portalModal((0, react.createElement)("div", { className: "mkts-ob" }, (0, react.createElement)("div", {
		...
		*/
		...
		function InstalledPanel() {
			...
			(0, react.useEffect)(() => {
				...
				const iv = setTimeout(() => setData((d) => d.toast === data.toast ? {
					...d,
					toast: null
				} : d), 6e3);
				return () => {
			...
			return (0, react.createElement)("div", { className: "mkts" }, data.toast ? (0, react.createElement)("div", { className: data.toastKind === "ok" ? "mkts-toast" : "mkts-err" }, data.toast) : null, data.self && data.self.updateAvailable ? (0, react.createElement)("div", { className: "mkts-selfupdate" }, (0, react.createElement)("span", { style: { flex: 1 } }, fmt("selfUpdate", {
				...
				onClick: () => toggle(p)
			}, !!togglingSet[p.name] ? t("toggling") : p.enabled ? t("disable") : t("enable")), p.updateAvailable ? (0, react.createElement)("button", {
				className: "mkts-cmdbtn",
				disabled: opActive,
