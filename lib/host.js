...
function isRelativePathSpec(target) {
...
function sweepSnapshots(profile, keep = 3) {
    try {
...
function startOp(kind, profile, target, label, explicitBin, initialOutput) {
    if (activeOp && activeOp.status === 'running')
        return { ok: false, busy: true, error: '已有任务进行中：' + activeOp.label };
    ...
    child.stdout.on('data', (s) => { appendOutput(op, s); });
    child.stderr.on('data', (s) => { appendOutput(op, s); });
    child.on('error', (err) => {
...
function removeHotYml(file) {
    if (file === null)
        return;
    try {
        rmSync(file, { force: true, maxRetries: 3 });
    }
    catch { }
}
...
async function disposeHotMount(profile, packageName) {
 ...
 */
function liveLoaderStates(loader) {
    if (loader === undefined) {
        try {
...
function runCliSync(args, timeoutMs, target) {
    const inv = dshInvoke();
    if (!inv)
...
function marketJobHooks(op) {
    ...
    return {
        cancel: () => {
            ...
            if (activeOp === op && op.status === 'running')
                killOp();
        },
    ...
    try {
        const jobId = jobs.start({
            ...
            run: () => {
                ...
                if (!op || op.id !== opId)
                    throw new Error('任务启动异常：op 不匹配（' + opId + '）');
        ...
        async execute(args, exec) {
            ...
            if (jobs !== undefined) {
                ...
                const started = startMarketJob(jobs, exec, 'market-install', spec, () => {
                    ...
                    if (r.busy)
                        throw new Error((r.error || '已有任务进行中') + '（等待完成后再试）');
                    if (!r.ok || !r.opId)
                ...
                if (method === 'update') {
                    ...
                    const target = spec.startsWith('github:') ? spec.replace(/#.*$/, '')
                        : spec.startsWith('https://codeload.github.com/') ? spec.replace(/\/tar\.gz\/.+$/, '/tar.gz/HEAD')
                            : `${name}@latest`;
                    if (!safeCliSpec(target))
                        return sendJson(res, 200, { ok: false, output: '安装目标包含不支持的字符' });
